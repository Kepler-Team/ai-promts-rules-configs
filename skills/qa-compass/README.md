...
Codex can install the skill directly from the Kepler repository. In Codex, ask:
...
If an older local version exists, remove it first from:

```text
~/.codex/skills/qa-compass
```

Direct Codex installer command:

```bash
python3 ~/.codex/skills/.system/skill-installer/scripts/install-skill-from-github.py \
  --repo Kepler-Team/ai-promts-rules-configs \
...
Claude Code can use the same skill folder. Install this directory into Claude Code's skills directory:

```text
~/.claude/skills/qa-compass
```

After installation, restart Claude Code so it can load the updated skill.
